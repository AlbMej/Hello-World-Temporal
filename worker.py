...
async def main():
    ...
    async with Worker(  # https://python.temporal.io/temporalio.worker.Worker.html
        ...
        task_queue="greeting",  # Tells Temporal Server, "I am only set up to process tasks from this queue"
        workflows=[GreetingWorkflow],  # List of class references, written specifically to handle running activities
        activities=[compose_greeting],  # List of function references that process the tasks on the requested task_queue
...
if __name__ == "__main__":
    ...
    asyncio.run(main())  # Use simple_main() for a more basic example without async context manager